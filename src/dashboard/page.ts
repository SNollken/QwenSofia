...
:root{color-scheme:dark;--bg:#0b1018;--bg-soft:#111827;--panel:rgba(20,29,43,.92);--panel-strong:#182235;--line:#2a3850;--muted:#9aa9bf;--text:#f4f7fb;--accent-start:#7c8cff;--accent-end:#9c8cff;--accent-soft:rgba(124,140,255,.18);--green:#5fcf9c;--red:#e97887;--yellow:#d7b66f;--shadow:0 18px 42px rgba(2,6,15,.24)}
*{box-sizing:border-box}body{min-height:100vh;margin:0;background:radial-gradient(80rem 48rem at -12% -18%,rgba(76,105,164,.28),transparent 52%),radial-gradient(56rem 34rem at 108% 0,rgba(134,103,181,.16),transparent 56%),linear-gradient(145deg,#0b1018 0%,#0e1623 48%,#111827 100%);color:var(--text);font:14px ui-sans-serif,Segoe UI,Arial,sans-serif;letter-spacing:.005em}
button,input{font:inherit}
.top{min-height:76px;border-bottom:1px solid rgba(143,160,190,.16);display:flex;align-items:center;justify-content:space-between;padding:0 max(28px,calc((100vw - 1180px)/2));background:rgba(11,16,24,.76);backdrop-filter:blur(18px);position:sticky;top:0;z-index:2}
...
.endpoint{color:#c3cbe0;font:12px ui-monospace,monospace;border:1px solid rgba(146,161,190,.22);border-radius:999px;padding:6px 10px;background:rgba(24,34,51,.72)}
.nav{display:flex;gap:5px;padding:4px;border:1px solid rgba(146,161,190,.16);border-radius:12px;background:rgba(19,28,43,.7)}
.nav button,.ghost{color:#b8c4d6;background:transparent;border:1px solid transparent;border-radius:8px;padding:8px 13px;cursor:pointer;transition:.16s ease}
.nav button.active{color:#fff;border-color:rgba(150,157,255,.42);background:linear-gradient(135deg,rgba(124,140,255,.32),rgba(156,140,255,.22));box-shadow:inset 0 1px rgba(255,255,255,.14)}.nav button:hover,.ghost:hover{color:#fff;background:rgba(255,255,255,.06)}
.wrap{max-width:1180px;margin:0 auto;padding:34px 28px 48px}
.muted{color:var(--muted)}
.actions{display:flex;gap:9px;flex-wrap:wrap}
.btn{border:1px solid rgba(177,183,255,.35);border-radius:10px;padding:10px 14px;background:linear-gradient(135deg,var(--accent-start),var(--accent-end));color:#fff;font-weight:750;cursor:pointer;box-shadow:0 8px 18px rgba(90,105,211,.2);transition:transform .16s ease,filter .16s ease}.btn:hover{filter:brightness(1.07);transform:translateY(-1px)}.btn:disabled{cursor:wait;opacity:.7;transform:none}
.btn.secondary{color:#dce4f2;background:rgba(34,46,68,.82);border-color:#3a4b68;box-shadow:none}.btn.danger{background:rgba(105,43,58,.68);color:#ffe1e5;border-color:#804557;box-shadow:none}
.section-head{display:flex;align-items:end;justify-content:space-between;gap:16px;margin:0 0 16px;flex-wrap:wrap}.section-head h2{margin:0 0 5px;font-size:24px;letter-spacing:-.035em}.section-kicker{color:#aeb9ff;font-size:11px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;margin:0 0 8px}
.grid{display:grid;gap:10px}.accounts-grid{grid-template-columns:repeat(auto-fill,minmax(330px,1fr));align-items:stretch}
.card{border:1px solid rgba(143,160,190,.19);border-radius:15px;background:linear-gradient(145deg,rgba(27,39,57,.96),rgba(17,25,38,.96));padding:15px 16px;display:flex;align-items:center;justify-content:space-between;gap:14px;box-shadow:var(--shadow);transition:border-color .16s ease,transform .16s ease}.card:hover{border-color:rgba(150,157,255,.4);transform:translateY(-1px)}
...
.avatar{width:40px;height:40px;border-radius:13px;background:linear-gradient(145deg,#35456b,#293558);display:grid;place-items:center;font-weight:800;color:#e0e7ff;flex:0 0 auto}.account-email{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.account-id{font:11px ui-monospace,monospace;margin-top:4px;opacity:.75}
...
.empty{border:1px dashed rgba(143,160,190,.32);border-radius:15px;padding:24px;color:var(--muted);background:rgba(20,29,43,.55)}
...
input{background:#101927;border:1px solid var(--line);border-radius:8px;color:var(--text);padding:10px 12px}
...
.jobs{margin-top:36px;padding-top:30px;border-top:1px solid rgba(143,160,190,.16)}.metric-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(195px,1fr));gap:12px}.metric-card{position:relative;overflow:hidden;align-items:start;display:grid;gap:8px;min-height:144px;padding:18px}.metric-card::after{content:"";position:absolute;inset:0 0 auto;height:3px;background:linear-gradient(90deg,var(--accent-start),var(--accent-end))}.metric-card:nth-child(1)::after{background:var(--green)}.metric-card:nth-child(3)::after{background:var(--yellow)}.metric-value{font-size:27px;letter-spacing:-.04em}.metric-label{color:#bcc7d9;font-size:12px;font-weight:750;text-transform:uppercase;letter-spacing:.075em}.metric-action{align-self:end;justify-self:start;padding:0;border:0;background:transparent;color:#c7ceff;font-weight:750;cursor:pointer}.metric-action:hover{color:#fff;text-decoration:underline}
